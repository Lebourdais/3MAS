...
def gabor_impulse_response(t, center, fwhm):
    ...
    gaussian = torch.exp(
        torch.tensordot(
            1.0 / (2.0 * fwhm.unsqueeze(1) ** 2), (-(t**2.0)).unsqueeze(0), dims=1
        )
    )
    ...
    sinusoid = torch.exp(
        ...
        * torch.tensordot(
            center_frequency_complex.unsqueeze(1), t_complex.unsqueeze(0), dims=1
        )
    )
...
def gabor_filters(kernel, size: int = 401, legacy_complex=False):
    t = torch.arange(
        -(size // 2), (size + 1) // 2, dtype=kernel.dtype, device=kernel.device
    )
