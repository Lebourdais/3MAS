...
def get_frontend(opt):
    ...
    if "leaf" in front_end_config["name"].lower():
        ...
        if default_args:
            ...
            window_len_ms = float(audio_config.get("window_len", 25.0))
            window_stride_ms = float(audio_config.get("window_stride", 10.0))
    ...
    if pretrained_flag:
        print(
            "attempting to load pretrained frontend weights..", fe.load_state_dict(ckpt)
        )
    return fe
