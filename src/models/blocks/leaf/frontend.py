...
class Leaf(nn.Module):
    def __init__(
        ...
        if initializer == "default":
            initializer = GaborInit(
                default_window_len=window_size,
                sample_rate=sample_rate,
                min_freq=init_min_freq,
                max_freq=init_max_freq,
            )
        self._complex_conv = GaborConv1d(
            filters=2 * n_filters,
        ...
        self._pooling = GaussianLowPass(
            n_filters, kernel_size=window_size, strides=window_stride, padding="same"
        )
        self._instance_norm = None
        if mean_var_norm:
            raise NotImplementedError("Instance Norm functionality not added yet..")
        if pcen_compression:
            self._compression = PCENLayer(
                n_filters,
                alpha=0.96,
                smooth_coef=0.04,
                delta=2.0,
